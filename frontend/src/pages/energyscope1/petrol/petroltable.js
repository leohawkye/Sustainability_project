...
import PetrolAdd from "./petroladd";
...
const PetrolTable = () => {
  ...
  const deletePet = async id => {
    try {
      const deletePet = await fetch(`http://localhost:5000/petrol/${id}`, {
        method: "DELETE"
      });
  ...
  const getPet = async () => {
    try {
      const response = await fetch("http://localhost:5000/petrol");
